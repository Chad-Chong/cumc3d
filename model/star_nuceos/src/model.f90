...
WRITE (*,*) 'EOS file = ', default_eos
CALL readtable(default_eos)
WRITE (*,*)
WRITE (*,*) 'Finished reading Nuclear table'
...
DO j = 1, nx
  DO k = 1, ny
    ...
    temperature(j,k,1) = max(temperature(j,k,1), temp_a)
    CALL nuc_eos_one(prim(irho,j,k,1)/rhocgs2code, temperature(j,k,1), prim(iye,j,k,1),prscgs,1)
    CALL nuc_eos_one(prim(irho,j,k,1)/rhocgs2code, temperature(j,k,1), prim(iye,j,k,1),epscgs,2)
    prim(itau,j,k,1) = prscgs*taucgs2code
    epsilon(j,k,1)   = epscgs*energycgs2code
  END DO
END DO
