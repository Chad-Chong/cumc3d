...
SUBROUTINE buildThermalNeutrino
USE DEFINITION
USE CUSTOM_DEF
IMPLICIT NONE

ALLOCATE(Q_nudot(-2:nx+3,-2:ny+3,-2:nz+3)) 
...
! Calculate the thermal neutrino heat loss

REAL*8 :: temp, den, snu, dummy
INTEGER :: i,j,k,l
      ...
      DO k = 1, ny, 1
            DO j = 1, nx, 1
                  IF (prim(irho,j,k,l) > prim_a(irho)) THEN
                        ...
                        temp = temp2(j,k,l)*1.0D9
                        den = prim(irho,j,k,l)/rhocgs2code

                        CALL sneut5_aa(temp, den, abar2(j,k,l), zbar2(j,k,l), &
                              snu,dummy,dummy,dummy,dummy, &
                              dummy,dummy,dummy,dummy,dummy)
                        
                        Q_nudot(j,k,l) = prim(irho,j,k,l) * (snu * (energycgs2code/masscgs2code/tcgs2code) )
                        sc(itau,j,k,l) =  sc(itau,j,k,l) - Q_nudot(j,k,l)
