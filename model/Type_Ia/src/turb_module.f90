...
CONTAINS
   ...
   do k = 1, nz, 1
      do j = 1, ny, 1
         do i = 1, nx, 1
            ...
            turb_div_v(i,j,k) = 2.0D0/x(i)*prim(ivx,i,j,k) + turb_velgrad(i,j,k,1,1) + 1/x(i)/sine(j)*(DCOS(y(j))*prim(ivy,i,j,k))+ 1/x(i)* dvydy + 1/x(i)/sine(j)*0.5D0 * dvzdz
   ...
   do k = 1, nz, 1
      do j = 1, ny, 1
         do i = 1, nx, 1
               ...
               turb_diff(i,j,k) = 1/x(i)*(2*turb_qdev(i,j,k,1))+dqdevdx + 1/x(i)/sine(j)*(DCOS(y(j))*turb_qdev(i,j,k,2)) + 1/x(i)*dqdevdy + 1/x(i)/sine(j)*dqdevdz
         enddo
      enddo
   enddo
   ...
   do k = 1, nz, 1
      do j = 1, ny, 1
         do i = 1, nx, 1
            turb_source(i,j,k) = -turb_comp(i,j,k) + turb_str(i,j,k) - turb_eps(i,j,k)
         enddo
      enddo
   enddo
